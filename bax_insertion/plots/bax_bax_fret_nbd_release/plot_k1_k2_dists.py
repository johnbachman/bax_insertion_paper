...
def parameter_table(filelist, prefix='pt_data1_newpr'):
    ...
    for filename in filelist:
        ...
        file_dict[(activator, residue, repnum)] = \
                        (np.mean(sampler.flatchain[0], axis=0), gf.data[0,0,0])
        # Store the keys
        activators.add(activator)
        nbd_residues.add(residue)
        replicates.add(repnum)
    ...
    with open('justin_table.csv', 'w') as f:
        ...
        csvwriter.writerow(['Activator', 'Residue', 'Rep', 'F0', 'k1', 'c1',
                            'k2', 'c2'])
...
if __name__ == '__main__':
    ...
    usage =  'Usage:\n'
    usage += '    python plot_k1_k2_dists.py assemble [k1|k2|c1|c2|f1|f2] ' \
                                    '[file glob]\n'
    usage += '    python plot_k1_k2_dists.py plot [k1k2|c1c2|f1f2] ' \
    ...
    if len(sys.argv) < 4:
        ...
        if plot_type not in ['k1k2', 'c1c2', 'f1f2']:
            print(usage)
            sys.exit(1)
